...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    ...
    scrollZoom: false //Where the map will start when the page is loaded:
  ...
  var bounds = new mapboxgl.LngLatBounds(); //Looping over the locations array to add a marker on the map:

  locations.forEach(function (loc) {
    //Adding the marker since we already have the css configurations for it:
    var el = document.createElement("div"); //The name in the styles.css is also marker:
    ...
    new mapboxgl.Marker({
      ...
      anchor: "bottom"
    }).setLngLat(loc.coordinates).addTo(map); //Adding the popup
...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert(); //Adding new one:

  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>");
  document.querySelector("body").insertAdjacentHTML("afterbegin", markup); //Removing after 5sec:
        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === "success") {
              (0, _alert.showAlert)("success", "Logged in");
